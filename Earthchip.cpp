...
Earthchip::Earthchip(const char* domain, uint16_t port, Client& client) {
    ...
    setKeepAlive(MQTT_KEEPALIVE);
    setSocketTimeout(MQTT_SOCKET_TIMEOUT);
    this->node_id = retrieveNodeID();
    connect(DEFAULT_DEVICE_ID);
}
...
/*BASIC INTERFACING METHODS. These functions provide the minimal setup to push data into
the Earthchip Dashboard*/

//TEMPERATURE - PUSH SENSOR READINGS AS DOUBLE
boolean Earthchip::pushTemperature(double reading) {
    String strTopic = node_id + TEMPERATURE_TOPIC; 
    return publish(strTopic.c_str(), String(reading).c_str());
}

//TEMPERATURE - PUSH SENSOR READINGS AS CHAR*
boolean Earthchip::pushTemperature(const char* reading) {
    String strTopic = node_id + TEMPERATURE_TOPIC; 
    return publish(strTopic.c_str(), reading);
}

//MOISTURE - PUSH SENSOR READINGS AS DOUBLE
boolean Earthchip::pushMoisture(double reading) {
    if (reading < 0) {
        reading = 0;
    }
    if (reading > 100) {
    ...
    String strTopic = node_id + MOISTURE_TOPIC; 
    return publish(strTopic.c_str(), String(reading).c_str());
}

//MOISTURE - PUSH SENSOR READINGS AS CHAR*
boolean Earthchip::pushMoisture(const char* reading) {
    String strTopic = node_id + MOISTURE_TOPIC; 
    return publish(strTopic.c_str(), reading);
}

//HUMIDITY - PUSH SENSOR READINGS AS DOUBLE
boolean Earthchip::pushHumidity(double reading) {
    String strTopic = node_id + HUMIDITY_TOPIC; 
    return publish(strTopic.c_str(), String(reading).c_str());
}

//HUMIDITY - PUSH SENSOR READINGS AS CHAR*
boolean Earthchip::pushHumidity(const char* reading) {
    String strTopic = node_id + HUMIDITY_TOPIC; 
    return publish(strTopic.c_str(), reading);
}

//LIGHT - PUSH SENSOR READINGS AS DOUBLE
boolean Earthchip::pushLight(double reading) {
    String strTopic = node_id + HUMIDITY_TOPIC; 
    return publish(strTopic.c_str(), String(reading).c_str());
}

//LIGHT - PUSH SENSOR READINGS AS CHAR*
boolean Earthchip::pushLight(const char* reading) {
    String strTopic = node_id + HUMIDITY_TOPIC; 
    return publish(strTopic.c_str(), reading);
}


//METHOD TO RETRIEVE THE INTERNAL NODE IDENTIFIER FOR THE ESP8266 INTERFACE CARD
String Earthchip::retrieveNodeID () {
    if (mountFS()) {
        if(readFS()== "") {
            return writeFS();
        } return readFS();
...
//WRITE RANDOM INTERNAL NODE IDENTIFIER IF NOT PRESENT
String Earthchip::writeFS() {
    ...
    int bytesWritten = file.print(PLANT_NAMES[(rand() % sizeof(PLANT_NAMES -1)) + 1]);
